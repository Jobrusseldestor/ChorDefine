...
class RealTimeScreenState extends State<RealTimeScreen> {
  ...
  Future<void> _initializeVision() async {
    ...
    try {
      await vision.loadYoloModel(
        labels: 'lib/model/ForeignTraining.txt',
        modelPath: 'lib/model/ForeignYolo_16.tflite',
        modelVersion: "yolov8",
        quantization: false,
        numThreads: 1,
        useGpu: false,
  ...
  Future<void> _loadModel() async {
    logger.i("Loading fraud detection model...");
    try {
      ...
      _interpreter = await Interpreter.fromAsset(
        'lib/model/BinaryCNN.tflite',
        options: interpreterOptions,
      );
      logger.i("Fraud detection model loaded successfully.");
    } catch (e) {
      logger.e("Failed to load CNN model", error: e);
    }
  }

  Future<void> _loadLabels() async {
    logger.i("Loading labels for fraud detection model...");
    try {
      final labelsData =
          await rootBundle.loadString('lib/model/BinaryCNNLabels.txt');
      _labels = labelsData.split('\n');
      logger.i("Labels loaded successfully: $_labels");
    } catch (e) {
      logger.e("Failed to load labels", error: e);
    }
  }
...
Future<void> _processDetections(List<Map<String, dynamic>> results) async {
  ...
  for (var res in results) {
    ...
    if (monetaryValue == "Foreign Banknote") {
      ...
      continue; // Skip CNN processing for Foreign Banknote
    }
